<img width="367" height="706" alt="image" src="https://github.com/user-attachments/assets/174b7a69-9926-4c86-9870-e3fc829a2f7c" /># 냉장고 파먹기 레벨업 챌린지 — Make & Zapier 자동화 구현 가이드 (디스코드 알림 버전)
...
## 4. Make
(
### 4-1. 시나리오 1 — 참가 신청이 오면 환영 인사 + 첫 미션 보내기(make와 zapier 비교용)
...
### 4-2. 시나리오 2 — 미션 인증이 오면 검사하고, 레벨별로 축하 보내기 (핵심!)
...
### 4-3. 확장 과제 — 전부 완료한 사람에게 인증서 + 초대장 보내기
...
```
[인증서](https://docs.google.com/presentation/d/1gdygC-RCrOYcWZEQqXNjgbTRMNeNU7IGPVv20YGr5cs/edit?usp=sharing)
...
> 💰 **무료 Task 아끼는 요령:** Zapier 무료는 월 100 Tasks뿐입니다. 참가자 1명이 완주하면 약 15 Tasks가 소모돼요. 
---
...
> 💡 **보너스 과제 아이디어:** 
① AI 연동 — 시나리오 2의 축하 메시지 앞에 OpenAI/Claude 모듈을 넣어, 참가자가 폼에 적은 요리 설명을 "할머니 말투 한 줄 칭찬"으로 바꿔 메시지에 붙이면 보너스 1 충족. 
② 실패 알림 — 시나리오 우측 상단 설정에서 오류 발생 시 관리자에게 이메일/디스코드 알림을 보내는 오류 처리(Error handler)를 추가하면 보너스 2 충족.

---

## 8. 자주 막히는 부분 Q&A

| 이런 문제가 생기면 | 이렇게 해결했어요. |
|--------------------|-------------------|
| 폼을 제출했는데 아무 일도 안 일어나요 | 참가자ID를 추가했어요 |
| 신청서 폼과 미션 수행폼을 한 번에 수했을 했더니 "환영인사가 없어요" | 신청서 폼과 미션수행 폼n | |
| 트래커에서 참가자 줄을 못 찾아요 | 라우터를 2개로 구성했어요 |
| 제미나이의 모델을 수정을 프롬프트가 사라져버려요 | 해결 못함 |
